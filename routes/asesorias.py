  ...
  if connection:
      ...
      cursor.execute("""
          SELECT a.codigo_asesoria, a.fecha_asesoria, a.asesor_asignado,
              ...
              a.especialidad, 
              p.metodo_pago, 
              COALESCE(pa.monto, 100.00) as precio,
...
# Rutas para el Chat con CometChat
@asesorias_bp.route('/chat/obtener_mensajes/<int:codigo_asesoria>')
@role_required('Usuario')
def obtener_mensajes_chat(codigo_asesoria):
    if 'user_id' not in session:
        return jsonify({'error': 'No autorizado'}), 401
    ...
    try:
        ...
        if connection:
            ...
            cursor.execute("""
                SELECT m.*, 
                       CASE 
                           WHEN m.tipo_emisor = 'solicitante' THEN CONCAT(u.nombres, ' ', u.apellidos)
                           WHEN m.tipo_emisor = 'asesor' THEN CONCAT(a.nombre, ' ', a.apellidos)
                       END as nombre_emisor
                FROM tbl_mensajes_chat m
                LEFT JOIN tbl_usuario u ON m.id_emisor = u.id_usuario AND m.tipo_emisor = 'solicitante'
                LEFT JOIN tbl_asesor a ON m.id_emisor = a.id_asesor AND m.tipo_emisor = 'asesor'
            ...
            cursor.execute("""
                UPDATE tbl_mensajes_chat 
                SET leido = 1 
                WHERE codigo_asesoria = %s AND tipo_emisor = 'asesor'
            """, (codigo_asesoria,))
...
@asesorias_bp.route('/chat/enviar_mensaje', methods=['POST'])
@role_required('Usuario')
def enviar_mensaje_chat():
    ...
    try:
        ...
        if not codigo_asesoria or not mensaje:
            return jsonify({'error': 'Faltan parámetros requeridos'}), 400
        ...
        if connection:
            ...
            # Verificar que la asesoría pertenece al usuario actual
            cursor.execute("""
                SELECT a.codigo_asesoria, s.id_solicitante
                FROM tbl_asesoria a
                JOIN tbl_solicitante s ON a.id_solicitante = s.id_solicitante
                WHERE a.codigo_asesoria = %s AND s.id_usuario = %s
            """, (codigo_asesoria, session['user_id']))
            ...
            # Insertar el mensaje
            cursor.execute("""
                INSERT INTO tbl_mensajes_chat (codigo_asesoria, id_emisor, tipo_emisor, mensaje, fecha_envio)
                VALUES (%s, %s, 'solicitante', %s, NOW())
            """, (codigo_asesoria, session['user_id'], mensaje))
            
            mensaje_id = cursor.lastrowid
            connection.commit()
            
            # Obtener el mensaje insertado con información del emisor
            cursor.execute("""
                SELECT m.*, CONCAT(u.nombres, ' ', u.apellidos) as nombre_emisor
                FROM tbl_mensajes_chat m
                JOIN tbl_usuario u ON m.id_emisor = u.id_usuario
                WHERE m.id_mensaje = %s
            """, (mensaje_id,))
            
            mensaje_enviado = cursor.fetchone()
            cursor.close()
            connection.close()
            
            # Convertir datetime a string
            if mensaje_enviado['fecha_envio']:
                mensaje_enviado['fecha_envio'] = mensaje_enviado['fecha_envio'].strftime('%Y-%m-%d %H:%M:%S')
            
            return jsonify({'success': True, 'mensaje': mensaje_enviado})
        else:
            return jsonify({'error': 'Error de conexión a la base de datos'}), 500
    except Exception as e:
