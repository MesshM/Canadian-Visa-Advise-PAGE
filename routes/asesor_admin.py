...
def asesorias_admin():
    ...
    if connection:
        with connection.cursor(dictionary=True) as cursor:
            # Primero, obtener el ID del asesor basado en el ID de usuario en la sesión
            cursor.execute("""
            ...
            # Obtener información del asesor para mostrar en la interfaz
            asesor_info = None
            if asesor_data and asesor_data['id_asesor']:
                cursor.execute("""
                    SELECT id_asesor, nombre, apellidos, correo 
                    FROM tbl_asesor 
                    WHERE id_asesor = %s
                """, (asesor_data['id_asesor'],))
                asesor_info = cursor.fetchone()

            if asesor_data and asesor_data['id_asesor']:
                # Si es un asesor, mostrar solo sus asesorías
                cursor.execute("""
                ...
                # Si es un administrador general, mostrar todas las asesorías
                cursor.execute("""
                    SELECT a.codigo_asesoria, a.fecha_asesoria, a.tipo_asesoria,
                           CONCAT(u.nombres, ' ', u.apellidos) AS solicitante,
                           u.correo,
            ...
            for asesoria in asesorias:
                if asesoria['fecha_asesoria'] and isinstance(asesoria['fecha_asesoria'], datetime.datetime):
                    asesoria['fecha_formateada'] = asesoria['fecha_asesoria'].strftime('%d %b, %Y')
                    asesoria['hora_formateada'] = asesoria['fecha_asesoria'].strftime('%H:%M')
                else:
                    asesoria['fecha_formateada'] = 'No disponible'
                    asesoria['hora_formateada'] = ''
        ...
        # Determinar si estamos mostrando asesorías filtradas por asesor
        es_vista_filtrada = asesor_data and asesor_data['id_asesor']
        
        return render_template(
            'asesor/asesorias_admin.html', 
            asesorias=asesorias, 
            clientes=clientes,
            es_vista_filtrada=es_vista_filtrada,
    ...
    try:
        ...
        if connection:
            with connection.cursor(dictionary=True) as cursor:
                ...
                # Condición adicional para filtrar por asesor si es necesario
                filtro_asesor = ""
                params_asesor = []
                
                if asesor_data and asesor_data['id_asesor']:
                    filtro_asesor = "AND a.id_asesor = %s"
                    params_asesor = [asesor_data['id_asesor']]
                ...
                try:
                    ...
                    # Total appointments (filtrado por asesor si es necesario)
                    try:
                        query = """
                            SELECT COUNT(*) as total FROM tbl_asesoria a
                    ...
                    # Calculate appointment growth (filtrado por asesor si es necesario)
                    try:
                        ...
                        query = """
                            ...
                            WHERE fecha_asesoria BETWEEN %s AND %s
                        """
                        params = [previous_month_start + " 00:00:00", previous_month_end + " 23:59:59"]
                    ...
                    # Paid appointments (filtrado por asesor si es necesario)
                    try:
                        query = """
                            SELECT COUNT(*) as total FROM tbl_asesoria a
                    ...
                    # Calculate paid appointments growth (filtrado por asesor si es necesario)
                    try:
                        query = """
                            SELECT COUNT(*) as total FROM tbl_asesoria a
                            WHERE estado = 'Pagada' AND fecha_asesoria BETWEEN %s AND %s
                    ...
                    # Total income (filtrado por asesor si es necesario)
                    try:
                        query = """
                    ...
                    # Calculate income growth (filtrado por asesor si es necesario)
                    try:
                        query = """
                            SELECT SUM(p.monto) as total FROM tbl_pago_asesoria p
                            JOIN tbl_asesoria a ON p.codigo_asesoria = a.codigo_asesoria
                    ...
                    # Appointments by type (filtrado por asesor si es necesario)
                    try:
                        # Visa de Trabajo
                        query = """
                            SELECT COUNT(*) as total FROM tbl_asesoria a
                    ...
                    # Appointments by month (filtrado por asesor si es necesario)
                    try:
                        ...
                        query += " GROUP BY MONTH(fecha_asesoria)"
                    ...
                    # Recent appointments (filtrado por asesor si es necesario)
                    try:
                        query = """
                            SELECT a.codigo_asesoria, a.fecha_asesoria, a.tipo_asesoria,
                                CONCAT(u.nombres, ' ', u.apellidos) AS solicitante,
                        ...
                        if filtro_asesor:
                            query += " WHERE " + filtro_asesor.replace("AND ", "")
                            params.extend(params_asesor)
                            
                        query += " ORDER BY a.fecha_asesoria DESC LIMIT 10"
...
def crear_asesoria():
    ...
    try:
        ...
        if connection:
            with connection.cursor(dictionary=True) as cursor:
                # Obtener el ID del asesor basado en el ID de usuario en la sesión
                cursor.execute("""
                    SELECT id_asesor, nombre, apellidos FROM tbl_asesor 
                    WHERE id_usuario = %s
                """, (session['user_id'],))
                asesor_data = cursor.fetchone()
                
                # Si es un asesor específico, asignar la asesoría a ese asesor
                if asesor_data and asesor_data['id_asesor']:
                    asesor_id = asesor_data['id_asesor']
                    asesor_nombre = f"{asesor_data['nombre']} {asesor_data['apellidos']}"
                    
                    cursor.execute("""
                        INSERT INTO tbl_asesoria 
                        (codigo_asesoria, id_solicitante, fecha_asesoria, tipo_asesoria, estado, descripcion, id_asesor, asesor_asignado) 
                        VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                    """, (codigo_asesoria, client_id, fecha_asesoria, asesoria_type, 'Pendiente', description, asesor_id, asesor_nombre))
                else:
                    # Si es un administrador general, crear la asesoría sin asignar a un asesor específico
                    cursor.execute("""
                        INSERT INTO tbl_asesoria 
...
def actualizar_estado_asesoria():
    ...
    try:
        ...
        if connection:
            with connection.cursor(dictionary=True) as cursor:
                # Verificar si el asesor tiene permiso para actualizar esta asesoría
                cursor.execute("""
                    SELECT id_asesor FROM tbl_asesor 
                    WHERE id_usuario = %s
                """, (session['user_id'],))
                ...
                if asesor_data and asesor_data['id_asesor']:
                    # Verificar si la asesoría pertenece a este asesor
                    cursor.execute("""
                        SELECT codigo_asesoria FROM tbl_asesoria 
                        WHERE codigo_asesoria = %s AND id_asesor = %s
                    """, (appointment_id, asesor_data['id_asesor']))
                    asesoria = cursor.fetchone()
                    
                    if not asesoria:
                        flash('No tienes permiso para actualizar esta asesoría', 'error')
                        return redirect(url_for('admin.asesorias_admin'))
...
def obtener_asesoria(codigo_asesoria):
    ...
    if connection:
        with connection.cursor(dictionary=True) as cursor:
            # Verificar si el asesor tiene permiso para ver esta asesoría
            cursor.execute("""
                SELECT id_asesor FROM tbl_asesor 
                WHERE id_usuario = %s
            """, (session['user_id'],))
            ...
            if asesor_data and asesor_data['id_asesor']:
                # Si es un asesor específico, verificar que la asesoría le pertenezca
                cursor.execute("""
                    SELECT a.codigo_asesoria, a.fecha_asesoria, a.tipo_asesoria,
                           CONCAT(u.nombres, ' ', u.apellidos) AS solicitante,
                ...
                """, (codigo_asesoria, asesor_data['id_asesor']))
            else:
                # Si es un administrador general, puede ver cualquier asesoría
                cursor.execute("""
                    SELECT a.codigo_asesoria, a.fecha_asesoria, a.tipo_asesoria,
                           CONCAT(u.nombres, ' ', u.apellidos) AS solicitante,
                           u.correo, a.estado, a.descripcion
        ...
        return jsonify({'error': 'Asesoría no encontrada o no tienes permiso para verla'}), 404
...
def obtener_cliente(cliente_id):
    ...
    if connection:
        with connection.cursor(dictionary=True) as cursor:
            ...
            # Verificar si el asesor tiene permiso para ver las asesorías de este cliente
            cursor.execute("""
                SELECT id_asesor FROM tbl_asesor 
                WHERE id_usuario = %s
            """, (session['user_id'],))
            ...
            # Get client appointments (filtradas por asesor si es necesario)
            if asesor_data and asesor_data['id_asesor']:
                cursor.execute("""
                    SELECT codigo_asesoria, tipo_asesoria, fecha_asesoria, estado
                    FROM tbl_asesoria
                    WHERE id_solicitante = %s AND id_asesor = %s
                    ORDER BY fecha_asesoria DESC
                """, (cliente_id, asesor_data['id_asesor']))
            else:
                cursor.execute("""
                    SELECT codigo_asesoria, tipo_asesoria, fecha_asesoria, estado
                    FROM tbl_asesoria
