...
function ordenarYNumerarAsesorias() {
  ...
  rows.sort((a, b) => {
      const idA = parseInt(a.getAttribute("data-asesoria-id"));
      const idB = parseInt(b.getAttribute("data-asesoria-id"));
      return idB - idA; // Orden descendente
  });

  // Reorganizar filas en el DOM sin borrar contenido
  rows.forEach((row, index) => {
      row.querySelector(".numero-asesoria").textContent = `#${rows.length - index}`; // Asigna número inverso
...
document.addEventListener("DOMContentLoaded", ordenarYNumerarAsesorias);
...
function editAsesoria(id) {
  // Verificar si la asesoría está vencida o pagada
  const row = document.querySelector(`tr[data-asesoria-id="${id}"]`)
  if (row) {
    if (row.getAttribute("data-estado") === "vencida") {
      showNotification("No se puede editar una asesoría vencida", "error")
      return
    }

    if (row.getAttribute("data-pago-estado") === "Pagada") {
      showNotification("No se puede editar una asesoría que ya ha sido pagada", "error")
      return
    }
  }
  ...
  if (row) {
    ...
    // Obtener el tipo de asesoría
    const tipoAsesoria = row.getAttribute("data-tipo-asesoria")
    if (tipoAsesoria) {
...
// Función para abrir el modal de pago con Stripe
function pagarAsesoria(id, tipoAsesoria) {
  document.getElementById("pago-asesoria-id").textContent = id
  document.getElementById("pago_codigo_asesoria").value = id
  document.getElementById("pago_tipo_asesoria").value = tipoAsesoria

  // Inicializar Stripe y crear el formulario de pago
  initializeStripe(id)
...
async function initializeStripe(asesoriaId) {
  try {
    ...
    const response = await fetch("/create-payment-intent", {
      ...
      body: JSON.stringify({
        ...
        tipo_asesoria: document.querySelector(`tr[data-asesoria-id="${asesoriaId}"]`)?.getAttribute("data-tipo-asesoria") || "Visa de Trabajo"
      }),
    })
    ...
    document.getElementById("monto").value = data.monto.toFixed(2)
    ...
    showNotification("Error al inicializar el formulario de pago: " + error.message, "error")
  }
}
...
async function handleSubmit(e) {
  ...
  try {
    ...
    if (error) {
      ...
      paymentMessage.classList.add("bg-red-100", "text-red-800", "border-l-4", "border-red-500")
      ...
      // Pago exitoso, actualizar la UI
      await procesarPago(document.getElementById("pago_codigo_asesoria").value)
    }
  } catch (error) {
    console.error("Error al procesar el pago:", error)

    // Mostrar mensaje de error
    paymentMessage.textContent = "Ocurrió un error al procesar el pago. Por favor, inténtalo de nuevo."
    paymentMessage.classList.remove("hidden")
    paymentMessage.classList.add("bg-red-100", "text-red-800", "border-l-4", "border-red-500")
            ...
            if (p.textContent.includes('Precio:')) {
              ...
              if (spanElement && !spanElement.querySelector("svg")) {
                spanElement.innerHTML = `
                  $${document.getElementById("monto").value} USD
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Mejorar la animación de los botones de pago
  document.querySelectorAll(".pagado-btn").forEach((btn) => {
    // Mejorar la animación al pasar el mouse
    btn.addEventListener("mouseenter", function () {
      const span = this.querySelector("span")
      if (span) {
